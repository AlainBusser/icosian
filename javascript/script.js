...
function prochain(x,y){                         // cherche le sommet le plus proche du lieu du clic
    var dmin = 1e6;
    var numero = -1;
...
/* fini pour les graphes, passons au jeu (Ouaaaaaais!) */
...
var lieu = 0;                                           // où c'est qu'elle est la tortue
var richesse = 0;                                            // la récolte

// définition du graphe: coordonnées des sommets et définition des arêtes
...
// le graphe est défini, on fait le jeu

var possible = [];                          // liste des sommets encore visitables
for (var n in abcs){
    possible[n] = true;
}

function essai(acteur,souris) {
    var oussamissava = prochain(souris.x, souris.y);    // destination visée
...
function manger(){                      // ramassage du rubis
    possible[lieu] = false;             // la tortue ne pourra plus revenir ici
    if (lieu>0) {
        gemmes[lieu].destroy();
        richesse++;
        aff.text = richesse;
...
var turtle, aff;
function preload(){                     // à faire avant de faire le jeu
...
function create(){                      // création du jeu
